...
public class ListRecentPurchases extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		listTransactions.setOnItemClickListener( new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View selectedView, int arg2,long arg3) {
				...
				Intent intent = new Intent(ListRecentPurchases.this, TransactionDetails.class);
	...
	public void onStart() {
		super.onStart();
		try {
			SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(ListRecentPurchases.this);
	        String username = prefs.getString("user", "anon");
			DBHelper dbhelper = new DBHelper(this);
			SQLiteDatabase db = dbhelper.getReadableDatabase();
			Cursor trans = db.rawQuery("select t._id,t.uid, item,quantity, transdate,transamount,transtype,cheque_no,cheque_party,cheque_details, t.remarks from transactions t inner join stock a  on ( a._id = t.account_id) where t.uid='"+username+"' AND transtype='Buying' order by  transdate desc   LIMIT 10",null);
			if ( trans.getCount() == 0 )
